# dashboard-nav-shell Specification

## Purpose
Define the shared dashboard navigation shell, including the persistent sidebar, top toolbar, PageShell integration, and responsive collapse behavior.
## Requirements
...
- **THEN** a left sidebar is visible containing navigation links to Articles, New Article, Vocabulary Review, Speaking Prompts, Exam Results, Users, and Ops
...
- **WHEN** the admin navigates to `/articles/[id]`, `/users/[id]`, or `/ops`
...
- **WHEN** an admin navigates to the Ops section
- **THEN** the toolbar title reads "Ops"

#### Scenario: Toolbar renders page-level actions when supplied
- **WHEN** a page provides a toolbar action
- **THEN** the action is rendered in the right side of the toolbar

#### Scenario: Toolbar title changes on navigation
- **WHEN** an admin navigates from Articles to Ops
- **THEN** the toolbar title updates to "Ops"
...
- **WHEN** a page mounts `<PageShell title="Ops">`
- **THEN** the toolbar displays "Ops" as the page title
