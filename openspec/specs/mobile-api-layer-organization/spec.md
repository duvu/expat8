# mobile-api-layer-organization Specification

## Purpose
Ensures the mobile API layer is organized with DTO classes extracted into dedicated model files and a shared request helper in `BackendApiClient`, so that `backend_api_client.dart` remains a thin HTTP client without embedded data models.
