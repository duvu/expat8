...
The mobile app SHALL derive displayed counts from local database state and SHALL update counts immediately after a successful add-word resolution that is treated as one learned vocabulary exposure, without waiting for backend sync.
...
#### Scenario: Stats update after add-word success
- **WHEN** the learner submits a word successfully through the immediate add-word flow
- **THEN** the progress stats page reflects one additional learned vocabulary item from local storage
