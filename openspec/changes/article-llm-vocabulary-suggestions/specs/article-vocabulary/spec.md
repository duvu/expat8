...
- **THEN** the response is `200` with `{ article_id, items: [{ term_id, display_term, word_sense_id, meaning_vi, part_of_speech, ipa, level, status, classification, suggestion_type }] }`
...
- **THEN** the response is `200` with the vocabulary items for that article, including any stored suggestion classification and level metadata
